...
def redistribute_commits(logs, num_commit):
	numbers_commits = gauss.generate_in_range(50, 60)
	if len(numbers_commits) != 29:
		raise("Incorrect number of commit numbers generated by the Gauss distribution.")
	total_commits = sum(numbers_commits)
	start_commit = num_commit - total_commits
	end_commit = num_commit
	(min_time, max_time) = compute_commit_time_period(logs, start_commit, end_commit)
	date = datetime.fromtimestamp(logs[start_commit]['author-date'])
	for i in range(0, 29):
		date_as_timestamp = int(time.mktime(date.date().timetuple()))
		for j in range(0, numbers_commits[i]):
			time = randint(min_time, max_time)
			timestamp = date_as_timestamp + time
